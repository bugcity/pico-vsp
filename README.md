...
| コマンド | 動作 |
|----------|------|
| `STATUS` | 現在のログ状態・フロー制御状態・信号線状態を返す |
| `FLOW ON` | RTSゲーティングを有効化（RTS=true のときだけ中継） |
| `FLOW OFF` | RTSゲーティングを無効化（デフォルト） |
...
# RTS/CTSハードウェアフロー制御を有効にする場合
uv run vsp.py --cdc2 /dev/cu.usbmodemXXXX --serial /dev/cu.usbserial-XXXX --rtscts
```
