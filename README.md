...
| ポート | 用途 | Mac ポート名 |
|--------|------|-------------|
| CDC0 | 制御ポート（コマンド受付） | `/dev/cu.usbmodemXXXX`（接続ごとに変わる） |
| CDC1 | デバイスA接続 | `/dev/cu.usbmodem1234563` |
| CDC2 | デバイスB接続 | `/dev/cu.usbmodem1234565` |
| CDC3 | キャプチャログ出力 | `/dev/cu.usbmodem1234567` |

CDC1 ↔ CDC2 は常時透過中継されます。CDC3 には通信ログが出力されます。

> **Note:** Mac のポート名はファームウェアのシリアル番号（`123456`）をベースに決まります。CDC0 のみ名前が接続のたびに変わるため、`ls /dev/cu.usbmodem*` で確認してください。
...
```shell
# CDC2ブリッジのみ
uv run vsp.py --cdc2 /dev/cu.usbmodem1234565 --serial /dev/cu.usbserial-XXXX

# CDC3キャプチャのみ
uv run vsp.py --cdc3 /dev/cu.usbmodem1234567 --log capture.log

# 両方同時
uv run vsp.py --cdc2 /dev/cu.usbmodem1234565 --serial /dev/cu.usbserial-XXXX \
              --cdc3 /dev/cu.usbmodem1234567 --log capture.log
